...
int main( int argc, char* args[] )
{
	...
	while( quit == false )
	{
		...
		std::stringstream moneyStreamShow;
		moneyStreamShow << currentMoney;
		cash = TTF_RenderText_Solid( font, moneyStreamShow.str().c_str(), textColor );

		//SDL_FillRect( screen, &screen->clip_rect, SDL_MapRGB( screen->format, 0xFF, 0xFF, 0xFF ) );
		currentMoney = currentMoney + 1;
		apply_surface( 0, 0, background, screen );
		apply_surface( 0, 150, cash, screen );
		myButton.show(); //Show the button
